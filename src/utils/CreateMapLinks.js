...
export const validateTravelType = validateEnum([
  'drive',
  'walk',
  'public_transport',
]);
export const validateMapType = validateEnum([
  'standard',
  'satellite',
  'hybrid',
  'transit',
...
const cleanObject = input => {
  return Object.keys(input).reduce((acc, key) => {
    ...
    return currentValue ? Object.assign({}, acc, {[key]: currentValue}) : acc;
  }, {});
};
...
export const createYandexParams = options => {
  ...
  if (options.start && !options.end) {
    console.warn(
      "Yandex Maps does not support current location, please specify direction's start and end.",
    );
...
export function createMapLink(options) {
  ...
  return (
    link[provider] + queryString.stringify(mapQueryParams).replace(/%2C/g, ',')
  );
}
